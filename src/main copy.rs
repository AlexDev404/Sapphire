...
// use core::iter::Once;
// use core::ptr;
// static mut VGA_ADDR: u32 = 0;

// fn init_vga_addr(framebuffer: u32) {
//     unsafe {
//         let fb_ptr: *mut u32 = framebuffer as *mut u32; // Example address
//         VGA_ADDR = ptr::read_volatile(fb_ptr);
//     }
// }

// fn get_vga_addr(framebuffer: u32) -> u32 {
//     INIT.call_once(|| {
//         init_vga_addr(framebuffer);
//     });
...
lazy_static! {
    ...
    let mut ptr: u32 = *VGA_ADDR;
    // asm!("mov {}, ebx", out(reg) ptr);
    // let fb = *VGA_ADDR as *mut u8;
    let fb = ptr as *mut u8;
    asm!("mov ax, 0x0F");
    asm!("add {}, 180050", in(reg) fb);
    asm!("mov [{}], ax", in(reg) fb);

    // // init_vga_addr(ptr.framebuffer);
    // let fb_addr: u32 = VGA_ADDR;
